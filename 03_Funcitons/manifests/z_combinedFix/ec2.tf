resource "aws_instance" "gjrEC2" {
    ...
    for_each = toset(keys({for az, details in data.aws_ec2_instance_type_offerings.my_instance_type_final: az => details.instance_types if length(details.instance_types) !=0 }))
